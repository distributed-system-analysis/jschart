...
function load_jschart_override_options() {
  ...
  if (imported_options.threshold_invalidate_on_load !== undefined) {
    if (
      imported_options.threshold_invalidate_on_load === "true" ||
      imported_options.threshold_invalidate_on_load === "TRUE"
    ) {
...
function datapoint(x, y, dataset, timestamp) {
  ...
  if (!this.dataset.max_y_value || this.dataset.max_y_value < this.y) {
...
function chart(
  ...
  this.x = {
    scale: { chart: null, zoom: null },
    brush: null,
    axis: { title: { text: x_axis_title, dom: null }, chart: null, zoom: null },
    slider: null
  };
  ...
  this.y = {
    scale: { chart: null, zoom: null },
    brush: null,
    axis: { title: { text: y_axis_title, dom: null }, chart: null, zoom: null },
    slider: null
  };
  ...
  this.chart = {
    ...
    coordinates: null,
    playpause: null,
    axis: { x: { chart: null, zoom: null }, y: { chart: null, zoom: null } },
  ...
  this.dom = {
    ...
    table: {
      ...
      view_port: {
        table: null,
        rows: { header: null, x: null, y: null, update: null },
        toggle_hide: null,
        inputs: {
          x: { domain: { min: null, max: null }, clamping: null },
          y: { domain: { min: null, max: null }, clamping: null }
        }
      },
      misc_controls: {
        table: null,
  ...
  this.table = { stacked_mean: 0, stacked_median: 0, stacked_value: 0 };
  ...
  this.state = {
    ...
    cursor_value: null,
    cursor_lock: false,
    cursor_lock_mouse_coordinates: { x: null, y: null },
    mouse: null,
  ...
  this.dimensions.viewport_width =
    this.dimensions.total_width -
    this.dimensions.margin.left -
    this.dimensions.margin.right;

  this.dimensions.viewport_height =
    this.dimensions.total_height -
    this.dimensions.margin.top -
    this.dimensions.margin.bottom;

  this.formatting = {
    table: { float: d3.format(" ,.2f"), integer: d3.format(" ,") },
    tooltip: d3.format(" ,f"),
    time: {
      utc: {
        long: d3.time.format.utc("%Y-%m-%d %H:%M:%S"),
        short: d3.time.format.utc("%M:%S")
      },
      local: {
        long: d3.time.format("%Y-%m-%d %H:%M:%S"),
  ...
  for (var i = 0; i < chart.datasets.valid.length; i++) {
    if (
      ...
      for (var x = 0; x < chart.datasets.valid[i].values.length; x++) {
        ...
        foo[chart.datasets.valid[i].values[x].x] +=
          chart.datasets.valid[i].values[x].y;
      }
    }
  }
...
function parse_plot_file(chart, datasets_index, text) {
  chart.datasets.all[datasets_index] = new dataset(
    datasets_index,
    "",
    "No Samples",
    "No Samples",
  ...
  for (var i = 0; i < file.length; i++) {
    ...
    if (index == -1) {
      if (chart.data_model == "histogram") {
        ...
        chart.datasets.all[datasets_index].histogram.sum +=
          +file[i][0] * +file[i][1];
      }
      chart.datasets.all[datasets_index].values.push(
        new datapoint(
          +file[i][0],
          +file[i][1],
          chart.datasets.all[datasets_index],
          null
        )
      );
    } else {
  ...
  if (chart.datasets.all[datasets_index].values.length > 0) {
    chart.datasets.all[datasets_index].mean = d3.mean(
      chart.datasets.all[datasets_index].values,
      get_datapoint_y
    );
    chart.datasets.all[datasets_index].median = d3.median(
      chart.datasets.all[datasets_index].values,
      get_datapoint_y
    );

    if (chart.data_model == "histogram") {
      chart.datasets.all[datasets_index].histogram.mean =
        chart.datasets.all[datasets_index].histogram.sum /
        chart.datasets.all[datasets_index].histogram.samples;
      chart.datasets.all[datasets_index].histogram.min =
        chart.datasets.all[datasets_index].values[0].x;
      chart.datasets.all[datasets_index].histogram.max =
        chart.datasets.all[datasets_index].values[
          chart.datasets.all[datasets_index].values.length - 1
        ].x;

      var count = 0;
      var threshold =
        chart.datasets.all[datasets_index].histogram.samples * 0.5;
      var threshold_p90 =
        chart.datasets.all[datasets_index].histogram.samples * 0.9;
      var threshold_p95 =
        chart.datasets.all[datasets_index].histogram.samples * 0.95;
      var threshold_p99 =
        chart.datasets.all[datasets_index].histogram.samples * 0.99;
      var threshold_p9999 =
        chart.datasets.all[datasets_index].histogram.samples * 0.9999;
      for (
        var i = 0;
        i < chart.datasets.all[datasets_index].values.length;
        i++
      ) {
        count += chart.datasets.all[datasets_index].values[i].y;
        if (
          chart.datasets.all[datasets_index].histogram.median === null &&
          count >= threshold
        ) {
          chart.datasets.all[datasets_index].histogram.median =
            chart.datasets.all[datasets_index].values[i].x;
        }
        if (
          chart.datasets.all[datasets_index].histogram.p90 === null &&
          count >= threshold_p90
        ) {
          chart.datasets.all[datasets_index].histogram.p90 =
            chart.datasets.all[datasets_index].values[i].x;
        }
        if (
          chart.datasets.all[datasets_index].histogram.p95 === null &&
          count >= threshold_p95
        ) {
          chart.datasets.all[datasets_index].histogram.p95 =
            chart.datasets.all[datasets_index].values[i].x;
        }
        if (
          chart.datasets.all[datasets_index].histogram.p99 === null &&
          count >= threshold_p99
        ) {
          chart.datasets.all[datasets_index].histogram.p99 =
            chart.datasets.all[datasets_index].values[i].x;
        }
        if (
          chart.datasets.all[datasets_index].histogram.p9999 === null &&
          count >= threshold_p9999
        ) {
          chart.datasets.all[datasets_index].histogram.p9999 =
            chart.datasets.all[datasets_index].values[i].x;
        }
        chart.datasets.all[datasets_index].values[i].percentile =
          (count / chart.datasets.all[datasets_index].histogram.samples) * 100;
      }
    }
  } else {
  ...
  if (
    chart.options.hide_dataset_threshold &&
    chart.datasets.all[i].max_y_value < chart.options.hide_dataset_threshold
  ) {
    if (
      jschart_override_options.threshold_invalidate_on_load !== undefined &&
...
function update_domains(chart) {
  ...
  if (!chart.state.custom_domain) {
    ...
    if (!chart.stacked && !chart.options.y.scale.log && domain[0] > 0) {
      domain[0] = 0;
    }
...
function update_chart(chart) {
  ...
  if (chart.stacked) {
    ...
    chart.dom.table.stacked.mean.text(
      table_print(chart, chart.table.stacked_mean)
    );

    compute_stacked_median(chart);
    chart.dom.table.stacked.median.text(
      table_print(chart, chart.table.stacked_median)
    );
  }
}
  ...
  d3.json(chart.options.json_plotfile)
    ...
    .post(post_data, function(error, json) {
      if (
        ...
        for (
          var index = 0, dataset_index = 0;
          index < json.data_series_names.length;
          index++
        ) {
          ...
          for (var i = 0; i < json.data.length; i++) {
            chart.datasets.valid[dataset_index].values.push(
              new datapoint(
                null,
                json.data[i][index],
                chart.datasets.valid[dataset_index],
                json.data[i][0]
              )
            );
            chart.datasets.valid[dataset_index].last_timestamp =
              json.data[i][0];
          }

          if (chart.options.history_length) {
            var delta =
              chart.datasets.valid[dataset_index].values.length -
              chart.options.history_length;
          ...
          for (
            var i = 0;
            i < chart.datasets.valid[dataset_index].values.length;
            i++
          ) {
            chart.datasets.valid[dataset_index].values[i].x =
              chart.datasets.valid[dataset_index].values[i].timestamp;
          }
          ...
          if (chart.datasets.valid[dataset_index].values.length > 0) {
            mean = d3.mean(
              chart.datasets.valid[dataset_index].values,
              get_datapoint_y
            );
            median = d3.median(
              chart.datasets.valid[dataset_index].values,
          ...
          chart.datasets.valid[dataset_index].dom.table.mean.text(
            chart.formatting.table.float(mean)
          );
          chart.datasets.valid[dataset_index].dom.table.median.text(
            chart.formatting.table.float(median)
          );
          chart.datasets.valid[dataset_index].dom.table.samples.text(
            chart.datasets.valid[dataset_index].values.length
          );
...
function parse_json(json, chart, callback) {
  if (
    ...
    for (
      ...
      chart.datasets.all[dataset_index] = new dataset(
        index - 1,
        json.data_series_names[index],
        0,
        0,
        [],
        chart
      ...
      for (var i = 0; i < json.data.length; i++) {
        chart.datasets.all[dataset_index].values.push(
          new datapoint(
            json.data[i][x_axis_index],
            json.data[i][index],
        ...
        chart.state.visible_datasets++;
        chart.datasets.all[dataset_index].last_timestamp =
          json.data[i][x_axis_index];
      }

      if (chart.datasets.all[dataset_index].values.length > 0) {
        chart.datasets.all[dataset_index].mean = d3.mean(
          chart.datasets.all[dataset_index].values,
          get_datapoint_y
        );
        chart.datasets.all[dataset_index].median = d3.median(
          chart.datasets.all[dataset_index].values,
          get_datapoint_y
        );
      } else {
...
function load_csv_text(text, chart, callback) {
  ...
  for (var x = 0; x < data.length; x++) {
    if (sample_counter == 0) {
      ...
      for (
        var i = 1, counter = 0;
        i < data[x].length;
        i += incrementer, counter++
      ) {
        chart.datasets.all[index_base + counter] = new dataset(
          index_base + counter,
          data[x][i],
          "No Samples",
          "No Samples",
      ...
      for (
        ...
        chart.datasets.all[index_base + counter].values.push(
          new datapoint(
            +data[x][ts_index],
            +data[x][i],
            chart.datasets.all[index_base + counter],
            null
          )
        );

        if (chart.data_model == "histogram") {
          chart.datasets.all[index_base + counter].histogram.samples += +data[
            x
          ][i];
          chart.datasets.all[index_base + counter].histogram.sum +=
            +data[x][ts_index] * +data[x][i];
        }
      }
    }
  ...
  for (var i = index_base; i < chart.datasets.all.length; i++) {
    if (chart.datasets.all[i].values.length) {
      chart.datasets.all[i].mean = d3.mean(
        chart.datasets.all[i].values,
        get_datapoint_y
      );
      chart.datasets.all[i].median = d3.median(
        chart.datasets.all[i].values,
        get_datapoint_y
      );

      if (chart.data_model == "histogram") {
        chart.datasets.all[i].histogram.mean =
          chart.datasets.all[i].histogram.sum /
          chart.datasets.all[i].histogram.samples;
        chart.datasets.all[i].histogram.min = chart.datasets.all[i].values[0].x;
        chart.datasets.all[i].histogram.max =
          chart.datasets.all[i].values[
            chart.datasets.all[i].values.length - 1
          ].x;
        ...
        for (var p = 0; p < chart.datasets.all[i].values.length; p++) {
          ...
          if (
            chart.datasets.all[i].histogram.median === null &&
            count >= threshold
          ) {
            chart.datasets.all[i].histogram.median =
              chart.datasets.all[i].values[p].x;
          }
          if (
            chart.datasets.all[i].histogram.p90 === null &&
            count >= threshold_p90
          ) {
            chart.datasets.all[i].histogram.p90 =
              chart.datasets.all[i].values[p].x;
          }
          if (
            chart.datasets.all[i].histogram.p95 === null &&
            count >= threshold_p95
          ) {
            chart.datasets.all[i].histogram.p95 =
              chart.datasets.all[i].values[p].x;
          }
          if (
            chart.datasets.all[i].histogram.p99 === null &&
            count >= threshold_p99
          ) {
            chart.datasets.all[i].histogram.p99 =
              chart.datasets.all[i].values[p].x;
          }
          if (
            chart.datasets.all[i].histogram.p9999 === null &&
            count >= threshold_p9999
          ) {
            chart.datasets.all[i].histogram.p9999 =
              chart.datasets.all[i].values[p].x;
          }
          chart.datasets.all[i].values[p].percentile =
            (count / chart.datasets.all[i].histogram.samples) * 100;
        }
      }
    } else {
      chart.datasets.all[i].invalid = true;
  ...
  d3.text(url, "text/plain").get(function(error, text) {
    ...
    for (var x = 0; x < data.length; x++) {
      if (sample_counter == 0) {
        ...
        for (var i = 0; i < data[x].length; i += 2) {
          if (
            data[x][i].startsWith("timestamp_") &&
            data[x][i].endsWith("_ms")
          ) {
            timestamp_columns++;
          }
        }
      ...
      if (chart.datasets.all[i].values.length) {
        ...
        if (chart.data_model == "histogram") {
          chart.datasets.all[i].histogram.mean =
            chart.datasets.all[i].histogram.sum /
            chart.datasets.all[i].histogram.samples;
          chart.datasets.all[i].histogram.min =
            chart.datasets.all[i].values[0].x;
          chart.datasets.all[i].histogram.max =
            chart.datasets.all[i].values[
              chart.datasets.all[i].values.length - 1
            ].x;
          ...
          var threshold_p99 = chart.datasets.all[i].histogram.samples * 0.99;
          var threshold_p9999 =
            chart.datasets.all[i].histogram.samples * 0.9999;
          for (var p = 0; p < chart.datasets.all[i].values.length; p++) {
            count += chart.datasets.all[i].values[p].y;
            if (
              chart.datasets.all[i].histogram.median === null &&
    ...
    if (packed_index == -1 && index >= 0) {
      ...
      while (packed_index >= 0) {
        ...
        parse_plot_file(
          ...
          dataset_index++,
          text.slice(
            prev_packed_index + packed_separator.length + 1,
            packed_index
          )
        );
      }
...
function create_chart_dataset_objects(chart) {
  ...
  chart.chart.legend
    ...
    .style("stroke", function(d) {
      return mycolors(d.index);
    });

  chart.chart.legend
    .append("rect")
    .classed("legendrect", function(d) {
      d.dom.legend.rect = d3.select(this);
      return true;
    })
  ...
  chart.chart.legend
    ...
    .attr("y", 13.5)
    .text(function(d) {
      d.dom.legend.label = d3.select(this);
      return d.name;
    });

  chart.chart.container.selectAll(".legendlabel").each(function(d, i) {
  ...
  if (chart.options.legend_entries) {
    var legend_entries = chart.chart.container
      ...
      .attr("transform", function(d, i) {
        return (
          "translate(" +
          (-chart.dimensions.margin.left + 5) +
          ", " +
          (chart.dimensions.viewport_height +
            chart.dimensions.legend_properties.margin.top +
            (Math.floor(
    ...
    legend_entries
      ...
      .attr("textLength", function(d, i) {
        if (
          d.length * chart.dimensions.pixels_per_letter >=
          chart.dimensions.total_width
        ) {
          return (chart.dimensions.total_width - 5).toFixed(0);
        }
      })
      .text(function(d) {
        return d;
  ...
  if (chart.stacked) {
    chart.chart.plot = chart.chart.container
      .selectAll(".plot")
    ...
    chart.chart.plot
      ...
      .attr("d", function(d) {
        ...
        return chart.functions.area(d.values);
      })
      .style("fill", function(d) {
        return mycolors(d.index);
      })
    ...
    for (var i = 0; i < chart.datasets.valid.length; i++) {
      ...
      chart.datasets.valid[i].dom.points = d3
        .select(chart.datasets.valid[i].dom.path[0][0].parentNode)
        .selectAll(".points")
    ...
    chart.chart.plot
      ...
      .attr("d", function(d) {
        ...
        return chart.functions.line(d.values);
      })
      .style("stroke", function(d) {
        return mycolors(d.index);
      })
    ...
    for (var i = 0; i < chart.datasets.valid.length; i++) {
      if (chart.datasets.valid[i].values.length > 1) {
        if (chart.options.scatterplot) {
          chart.datasets.valid[i].dom.markers = chart.chart.defs
            .append("marker")
            .attr("id", "dataset_" + chart.charts_index + "_" + i)
            .attr("viewBox", "-2,-2,4,4")
            .attr("markerWidth", 2)
            .attr("markerHeight", 2);
          chart.datasets.valid[i].dom.markers
            .append("circle")
            .style("fill", mycolors(i))
            .attr("r", 2);

          chart.datasets.valid[i].dom.path
            .classed("scatterplot", true)
            .attr("marker-mid", "url(#dataset_" + chart.charts_index + "_" + i)
            .attr(
              "marker-starter",
              "url(#dataset_" + chart.charts_index + "_" + i
            )
  ...
  chart.chart.cursor_points = chart.chart.container
    .append("g")
    .classed("cursor_points", true);

  for (var i = 0; i < chart.datasets.valid.length; i++) {
    chart.datasets.valid[i].dom.cursor_point = chart.chart.cursor_points
      .selectAll(".cursor_points")
...
function create_table(chart) {
  ...
  chart.dom.table.table = chart.dom.table.location
    .append("table")
    .classed("chart", true);
  ...
  chart.dom.table.threshold = cell
    .append("input")
    .attr("type", "text")
  ...
  chart.dom.table.name_filter = cell.append("input").attr("type", "text");

  cell
    .selectAll(".apply_name_filter_show")
    .data([chart])
    .enter()
    .append("button")
    .text("Show Datasets")
  ...
  cell
    .selectAll(".apply_name_filter_hide")
    .data([chart])
    .enter()
    .append("button")
    .text("Hide Datasets")
  ...
  if (chart.options.live_update) {
    ...
    chart.dom.table.live_update.history = cell
      .append("input")
      .attr("type", "text")
      .property("value", function() {
        if (chart.options.history_length) {
    ...
    cell
      ...
      .on("click", function() {
        ...
        if (!isNaN(value)) {
          ...
          chart.dom.table.live_update.history.property(
            "value",
            chart.options.history_length
          );
        }
    ...
    chart.dom.table.live_update.interval = cell
      .append("input")
      .attr("type", "text")
      .property("value", function() {
    ...
    cell
      ...
      .on("click", function() {
        ...
        if (!isNaN(value)) {
          ...
          if (chart.options.update_interval) {
            chart.dom.table.live_update.interval.property(
              "value",
              chart.options.update_interval
            );
          }
        }
      });
  ...
  chart.dom.table.view_port.table = cell
    .append("table")
    .attr("width", "100%")
    .classed("noborders", true);

  chart.dom.table.view_port.rows.header = chart.dom.table.view_port.table
    .selectAll(".view_port_table_controls")
    .data([chart])
    .enter()
    .append("tr")
    .on("click", toggle_hide_view_port_table_controls);

  chart.dom.table.view_port.toggle_hide = chart.dom.table.view_port.rows.header
    .append("th")
    .attr("colSpan", 4)
    .text("View Port Controls");

  chart.dom.table.view_port.rows.x = chart.dom.table.view_port.table
    .append("tr")
    .classed("controls", true);

  chart.dom.table.view_port.rows.x.append("th").text("X Axis");

  var cell = chart.dom.table.view_port.rows.x.append("td").text("Min: ");

  chart.dom.table.view_port.inputs.x.domain.min = cell
    .append("input")
    .attr("type", "text");

  var cell = chart.dom.table.view_port.rows.x.append("td").text("Max: ");

  chart.dom.table.view_port.inputs.x.domain.max = cell
    .append("input")
    .attr("type", "text");

  var cell = chart.dom.table.view_port.rows.x.append("td").text("Clamping: ");

  chart.dom.table.view_port.inputs.x.clamping = cell
    .selectAll(".x_clamp_checkbox")
    .data([chart])
    .enter()
    .append("input")
    .attr("type", "checkbox")
  ...
  chart.dom.table.view_port.rows.y = chart.dom.table.view_port.table
    .append("tr")
    .classed("controls", true);

  chart.dom.table.view_port.rows.y.append("th").text("Y Axis");

  var cell = chart.dom.table.view_port.rows.y.append("td").text("Min: ");

  chart.dom.table.view_port.inputs.y.domain.min = cell
    .append("input")
    .attr("type", "text");

  var cell = chart.dom.table.view_port.rows.y.append("td").text("Max: ");

  chart.dom.table.view_port.inputs.y.domain.max = cell
    .append("input")
    .attr("type", "text");

  var cell = chart.dom.table.view_port.rows.y.append("td").text("Clamping: ");

  chart.dom.table.view_port.inputs.y.clamping = cell
    .selectAll(".y_clamp_checkbox")
    .data([chart])
    .enter()
    .append("input")
    .attr("type", "checkbox")
  ...
  chart.dom.table.view_port.rows.update = chart.dom.table.view_port.table
    .append("tr")
    .classed("controls", true);

  var cell = chart.dom.table.view_port.rows.update
    .append("th")
    .attr("colSpan", 4);

  cell
    .selectAll(".updates_axes")
    .data([chart])
    .enter()
    .append("button")
    .text("Update Axes")
  ...
  chart.dom.table.misc_controls.table = cell
    .append("table")
    .attr("width", "100%")
    .classed("noborders", true);

  chart.dom.table.misc_controls.rows.header = chart.dom.table.misc_controls.table
    .selectAll(".misc_controls_table_controls")
    .data([chart])
    .enter()
    .append("tr")
    .on("click", toggle_hide_misc_controls_table_controls);

  chart.dom.table.misc_controls.toggle_hide = chart.dom.table.misc_controls.rows.header
    .append("th")
    .attr("colSpan", 4)
    .text("Misc. Controls");

  chart.dom.table.misc_controls.rows.sorting = chart.dom.table.misc_controls.table
    .append("tr")
    .classed("controls", true);

  chart.dom.table.misc_controls.rows.sorting.append("th").text("Sorting");

  var cell = chart.dom.table.misc_controls.rows.sorting
    .append("td")
    .attr("colSpan", 3)
    .text("Table Datasets: ");

  chart.dom.table.misc_controls.inputs.sort_datasets = cell
    .selectAll(".table_sort_datasets")
    .data([chart])
    .enter()
    .append("input")
    .attr("type", "checkbox")
  ...
  console.log(
    '...finished adding table controls for chart "' + chart.chart_title + '"'
  );
...
function table_add_datasets(chart) {
  for (var i = 0; i < chart.datasets.all.length; i++) {
    ...
    chart.datasets.all[i].dom.table.row
      .append("td")
      .attr("align", "left")
      .text(chart.datasets.all[i].name);

    chart.datasets.all[i].dom.table.value = chart.datasets.all[i].dom.table.row
      .append("td")
      .attr("align", "right");

    if (chart.data_model == "histogram") {
      chart.datasets.all[i].dom.table.percentile = chart.datasets.all[
        i
      ].dom.table.row
        .append("td")
        .attr("align", "right");
    ...
    chart.datasets.all[i].dom.table.mean = chart.datasets.all[i].dom.table.row
      .append("td")
      .attr("align", "right")
      .text(function() {
        if (chart.data_model == "histogram") {
    ...
    chart.datasets.all[i].dom.table.median = chart.datasets.all[i].dom.table.row
      .append("td")
      .attr("align", "right")
      .text(function() {
        if (chart.data_model == "histogram") {
    ...
    if (chart.data_model == "histogram") {
      chart.datasets.all[i].dom.table.histogram.min = chart.datasets.all[
        i
      ].dom.table.row
        .append("td")
        .attr("align", "right")
      ...
      chart.datasets.all[i].dom.table.histogram.max = chart.datasets.all[
        i
      ].dom.table.row
        .append("td")
        .attr("align", "right")
      ...
      chart.datasets.all[i].dom.table.histogram.p90 = chart.datasets.all[
        i
      ].dom.table.row
        .append("td")
        .attr("align", "right")
      ...
      chart.datasets.all[i].dom.table.histogram.p95 = chart.datasets.all[
        i
      ].dom.table.row
        .append("td")
        .attr("align", "right")
      ...
      chart.datasets.all[i].dom.table.histogram.p99 = chart.datasets.all[
        i
      ].dom.table.row
        .append("td")
        .attr("align", "right")
      ...
      chart.datasets.all[i].dom.table.histogram.p9999 = chart.datasets.all[
        i
      ].dom.table.row
        .append("td")
        .attr("align", "right")
      ...
      chart.datasets.all[i].dom.table.histogram.buckets = chart.datasets.all[
        i
      ].dom.table.row
        .append("td")
        .attr("align", "right")
        .text(
          chart.formatting.table.integer(chart.datasets.all[i].values.length)
        );
    }

    chart.datasets.all[i].dom.table.samples = chart.datasets.all[
      i
    ].dom.table.row
      .append("td")
      .attr("align", "right")
      .text(function() {
        if (chart.data_model == "histogram") {
          return chart.formatting.table.integer(
            chart.datasets.all[i].histogram.samples
          );
        } else {
          return chart.formatting.table.integer(
            chart.datasets.all[i].values.length
          );
        }
      });
  ...
  if (chart.options.raw_data_sources.length > 0) {
    var row = chart.dom.table.table.append("tr").classed("section", true);
    ...
    var cell = row.append("td").attr("colSpan", colspan);
    ...
    for (var i = 0; i < chart.options.raw_data_sources.length; i++) {
      cell
        ...
        .text(
          chart.options.raw_data_sources[i].substr(
            chart.options.raw_data_sources[i].lastIndexOf("/") + 1
          )
        )
        .append("br");
    }
...
function fix_y_axis_labels(chart) {
  chart.chart.axis.y.chart
    .selectAll("g.tick")
    .selectAll("text")
    .each(update_y_axis_label);
  chart.chart.axis.y.zoom
    .selectAll("g.tick")
    .selectAll("text")
    .each(update_y_axis_label);
}
...
function build_chart(chart) {
  ...
  chart.x.scale.zoom.clamp(true).range([0, chart.dimensions.viewport_width]);
  ...
  chart.y.scale.zoom.clamp(true).range([chart.dimensions.viewport_height, 0]);

  chart.x.axis.chart = d3.svg
    .axis()
    .scale(chart.x.scale.chart)
    .orient("bottom")
    .tickSize(-chart.dimensions.viewport_height);

  chart.x.axis.zoom = d3.svg
    .axis()
  ...
  chart.x.brush = d3.svg.brush().x(chart.x.scale.zoom);

  chart.y.axis.chart = d3.svg
    .axis()
    .scale(chart.y.scale.chart)
    .orient("left")
    .tickSize(-chart.dimensions.viewport_width);

  chart.y.axis.zoom = d3.svg
    .axis()
  ...
  if (!chart.options.scatterplot) {
    var bar = chart.chart.defs
      .append("marker")
      .attr("id", "datapoint_" + chart.charts_index)
      .attr("viewBox", "-2,-2,4,4")
      .attr("markerWidth", 2)
      .attr("markerHeight", 2);
    chart.chart.highlight_points = bar.append("circle").attr("r", 2);
  }

  chart.chart.container = chart.chart.svg
    .append("g")
    .attr(
      "transform",
      "translate(" +
  ...
  chart.chart.show_all = chart.chart.container
    .selectAll(".show")
    .data([chart])
    .enter()
    .append("text")
  ...
  chart.chart.hide_all = chart.chart.container
    .selectAll(".hide")
    .data([chart])
    .enter()
    .append("text")
  ...
  chart.chart.container
    ...
    .on("click", function() {
      ...
      for (var i = 0; i < x_values.length; i++) {
        ...
        if (x_values[i] >= x_domain[0] && x_values[i] <= x_domain[1]) {
          string = string + x_values[i] + ",";

          for (var d = 0; d < chart.datasets.all.length; d++) {
            for (
              var b = chart.datasets.all[d].tmp_index;
              b < chart.datasets.all[d].values.length;
              b++
            ) {
      ...
      for (var i = 0; i < chart.charts.length; i++) {
        ...
        if (chart.data_model == "timeseries") {
          if (chart.charts[i].data_model == "timeseries") {
            if (
              target_domain[0].getTime() !== source_domain[0].getTime() ||
          ...
          if (chart.charts[i].data_model == "timeseries") {
            ...
            if (
              target_domain[0] !== source_domain[0] ||
        ...
        if (domain_check) {
          console.warn(
            'Skipping application of X-Axis zoom from "' +
              chart.chart_title +
              '" to "' +
              chart.charts[i].chart_title +
              '" because data domains are not a match'
          );
          return;
        }
  ...
  chart.chart.axis.x.chart = chart.chart.container
    .append("g")
    .classed("axis", true)
    .attr("transform", "translate(0," + chart.dimensions.viewport_height + ")")
    .call(chart.x.axis.chart);

  chart.x.axis.title.dom = chart.chart.axis.x.chart
    .append("text")
  ...
  chart.chart.axis.x.zoom = chart.chart.container
    .append("g")
    .classed("axis", true)
  ...
  chart.x.slider = chart.chart.container
    .append("g")
    .classed("slider", true)
    .call(chart.x.brush);
  ...
  chart.chart.axis.y.chart = chart.chart.container
    .append("g")
    .classed("axis", true)
    .call(chart.y.axis.chart);

  chart.y.axis.title.dom = chart.chart.axis.y.chart
    .append("text")
  ...
  chart.chart.axis.y.zoom = chart.chart.container
    .append("g")
    .classed("axis", true)
    .attr(
      "transform",
      "translate(" + (chart.dimensions.viewport_width + 15) + ", 0)"
    )
  ...
  chart.y.slider = chart.chart.container
    .append("g")
    .classed("slider", true)
    .call(chart.y.brush);
  ...
  chart.y.slider
    ...
    .attr(
      "transform",
      "translate(" + (chart.dimensions.viewport_width + 5) + ", 0)"
    )
    .attr("width", 20);
  ...
  chart.chart.container
    ...
    .attr(
      "width",
      chart.x.scale.chart(x_domain[1]) - chart.x.scale.chart(x_domain[0])
    )
    .attr(
      "height",
  ...
  chart.chart.viewport = chart.chart.container
    .selectAll(".viewport")
    .data([chart])
    .enter()
    .append("rect")
...
exports.create_jschart = function(
  stacked,
  data_model,
  location,
  chart_title,
...
function click_highlight_function(dataset) {
  ...
  if (
    ...
    if (dataset.chart.state.chart_selection != -1) {
      dehighlight(
        dataset.chart.datasets.all[dataset.chart.state.chart_selection]
      );
      dataset.chart.datasets.all[
        dataset.chart.state.chart_selection
      ].highlighted = false;
    }
...
function highlight(dataset) {
  ...
  if (dataset.chart.stacked) {
    for (var i = 0; i < dataset.chart.datasets.valid.length; i++) {
      ...
      if (dataset.chart.datasets.valid[i].index == dataset.index) {
        dataset.chart.datasets.valid[i].dom.path.classed(
          "unhighlighted",
          false
        );

        if (dataset.chart.datasets.valid[i].dom.points) {
          dataset.chart.datasets.valid[i].dom.points.classed({
            unhighlighted: false,
            highlightedpoint: true
          });
        }

        dataset.chart.datasets.valid[i].dom.cursor_point.classed(
          "unhighlighted",
          false
        );
      } else {
    ...
    for (var i = 0; i < dataset.chart.datasets.valid.length; i++) {
      ...
      if (dataset.chart.datasets.valid[i].index == dataset.index) {
        if (dataset.chart.options.scatterplot) {
          ...
          dataset.chart.datasets.valid[i].dom.path
            .classed({ unhighlighted: false, highlightedline: true })
            .attr(
              "marker-mid",
              "url(#datapoint_" + dataset.chart.charts_index + ")"
            )
            .attr(
              "marker-start",
              "url(#datapoint_" + dataset.chart.charts_index + ")"
          ...
          dataset.chart.chart.highlight_points.style(
            "fill",
            mycolors(dataset.chart.datasets.valid[i].index)
          );
        }

        if (dataset.chart.datasets.valid[i].dom.points) {
          dataset.chart.datasets.valid[i].dom.points
  ...
  for (var i = 0; i < dataset.chart.datasets.valid.length; i++) {
    ...
    if (dataset.chart.datasets.valid[i].index == dataset.index) {
      dataset.chart.datasets.valid[i].dom.legend.rect.classed(
        "unhighlighted",
        false
      );
    } else {
  ...
  if (dataset.chart.stacked) {
    ...
    for (var i = 0; i < dataset.chart.datasets.valid.length; i++) {
      ...
      if (dataset.chart.options.scatterplot) {
        ...
        dataset.chart.datasets.valid[i].dom.path
          .classed({ unhighlighted: false, highlightedline: false })
...
function tooltip_on(d, i) {
  ...
  // will
  if (
    dimensions.x + dimensions.width + tooltip_margin >
    chart.dimensions.total_width
  ) {
    text.attr(
      "x",
      dimensions.x +
        (chart.dimensions.total_width -
          (dimensions.x + dimensions.width + tooltip_margin + 5))
    );
...
function set_x_axis_timeseries_label(chart) {
  ...
  if (chart.options.timezone == "local") {
    label +=
      "(" +
      chart.formatting.timezone(domain[0]) +
      "): " +
      chart.formatting.time.local.long(domain[0]) +
      " - " +
      chart.formatting.time.local.long(domain[1]);
  } else {
    label +=
      "(UTC/GMT): " +
      chart.formatting.time.utc.long(domain[0]) +
      " - " +
      chart.formatting.time.utc.long(domain[1]);
  }
...
function update_threshold_hidden_datasets(chart, field) {
  for (var i = 0; i < chart.datasets.valid.length; i++) {
    ...
    if (field == "max_y") {
      if (
        chart.datasets.valid[i].max_y_value <
        chart.options.hide_dataset_threshold
      ) {
        hidden = true;
      }
      ...
      if (chart.data_model == "histogram") {
        if (
          chart.datasets.valid[i].histogram.mean <
          chart.options.hide_dataset_threshold
        ) {
          hidden = true;
        }
        ...
        if (
          chart.datasets.valid[i].mean < chart.options.hide_dataset_threshold
        ) {
          hidden = true;
        }
      }
...
function update_dataset_chart_elements(chart) {
  if (chart.stacked) {
    for (var i = 0; i < chart.datasets.valid.length; i++) {
      ...
      if (chart.datasets.valid[i].dom.points) {
        chart.datasets.valid[i].dom.points
          .attr("x1", get_chart_scaled_x)
          .attr("x2", get_chart_scaled_x)
          .attr("y1", get_chart_scaled_y0)
          .attr("y2", get_chart_Scaled_y_y0);
...
function set_dataset_value(chart, dataset_index, values_index) {
  ...
  chart.datasets.valid[dataset_index].dom.table.value.text(
    chart.formatting.table.float(
      get_datapoint_y(chart.datasets.valid[dataset_index].values[values_index])
    )
  );
  if (chart.data_model == "histogram") {
    chart.datasets.valid[dataset_index].dom.table.percentile.text(
      chart.formatting.table.float(
        chart.datasets.valid[dataset_index].values[values_index].percentile
      )
    );
  }
  chart.datasets.valid[dataset_index].cursor_index = values_index;
  chart.table.stacked_value +=
    chart.datasets.valid[dataset_index].values[values_index].y;
  chart.datasets.valid[dataset_index].dom.cursor_point.data([
    chart.datasets.valid[dataset_index].values[values_index]
  ]);
  chart.datasets.valid[dataset_index].dom.cursor_point.attr(
    "cx",
    get_chart_scaled_x
  );
  chart.datasets.valid[dataset_index].dom.cursor_point.attr(
    "cy",
    get_chart_scaled_y_stack
  );
  if (!chart.datasets.valid[dataset_index].hidden) {
    chart.datasets.valid[dataset_index].dom.cursor_point.classed(
      "hidden",
      false
    );
  }
...
function show_dataset_values(chart, x_coordinate) {
  ...
  for (var i = 0; i < chart.datasets.valid.length; i++) {
    ...
    while (loop) {
      if (index == 0) {
        if (
          (chart.datasets.valid[i].values[index].x +
            chart.datasets.valid[i].values[index + 1].x) /
            2 >=
          x_coordinate
        ) {
          set = true;
  ...
  if (chart.stacked) {
    set_stacked_value(
      chart,
      chart.formatting.table.float(chart.table.stacked_value)
    );
  }
}
...
function get_chart_scaled_y_y0(datapoint) {
  return datapoint.dataset.chart.y.scale.chart(
    get_datapoint_y(datapoint) + datapoint.y0
  );
}
...
function viewport_mousemove(chart) {
  ...
  if (chart.chart.selection && chart.chart.selection.size() == 1) {
    var selection_x, selection_y, selection_width, selection_height;
  ...
  if (chart.state.cursor_lock) {
    mouse_values = [
      chart.x.scale.chart.invert(chart.state.cursor_lock_mouse_coordinates.x),
      chart.y.scale.chart.invert(chart.state.cursor_lock_mouse_coordinates.y)
    ];
  } else {
    mouse_values = [
      chart.x.scale.chart.invert(mouse[0]),
      chart.y.scale.chart.invert(mouse[1])
    ];
  }

  if (chart.data_model == "timeseries") {
    if (chart.options.timezone == "local") {
      chart.chart.coordinates.text(
        "x:" +
          chart.formatting.time.local.long(mouse_values[0]) +
          " y:" +
          chart.formatting.table.float(mouse_values[1])
      );
    } else {
      chart.chart.coordinates.text(
        "x:" +
          chart.formatting.time.utc.long(mouse_values[0]) +
          " y:" +
          chart.formatting.table.float(mouse_values[1])
      );
    }
    ...
    chart.chart.coordinates.text(
      "x:" +
        chart.formatting.table.float(mouse_values[0]) +
        " y:" +
        chart.formatting.table.float(mouse_values[1])
    );
  }
  ...
  chart.chart.xcursorline
    .attr("x1", chart.x.scale.chart(mouse_values[0]))
  ...
  chart.chart.ycursorline
    .attr("x1", chart.x.scale.chart(domain[0]))
...
function viewport_mousedown_primary(chart, mouse) {
  ...
  chart.chart.selection = chart.chart.container
    .insert("rect", "#coordinates")
...
function viewport_mouseup(chart) {
  ...
  if (d3.event.button == 0 && chart.state.selection_active) {
...
function viewport_mouseup_primary(chart, mouse) {
  ...
  if (
    chart.state.selection_start[0] == chart.state.selection_stop[0] ||
    chart.state.selection_start[1] == chart.state.selection_stop[1]
  ) {
    return;
  }

  var x_extent = Array(0, 0),
    y_extent = Array(0, 0);
...
function apply_y_max_threshold(chart) {
  ...
  if (!isNaN(value)) {
    ...
    chart.dom.table.threshold.property(
      "value",
      chart.options.hide_dataset_threshold
    );
  } else {
...
function update_axes_domains(chart) {
  var x_domain = [
    chart.dom.table.view_port.inputs.x.domain.min.property("value"),
    chart.dom.table.view_port.inputs.x.domain.max.property("value")
  ];
  var y_domain = [
    chart.dom.table.view_port.inputs.y.domain.min.property("value"),
    chart.dom.table.view_port.inputs.y.domain.max.property("value")
  ];
